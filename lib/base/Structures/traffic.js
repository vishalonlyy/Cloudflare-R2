...
class traffic {
     ...
     */
    delete(data) {
        let A_Data = Array.isArray(data) ? data : [data];
        if (!this.data.bucketName)
            throw new Error("Bucket name not set use .bucket() to set bucket name");
        return new Promise(async (resolve, reject) => {
            try {
                const result = await index_js_1.Utils.Delete(this.data.bucketName, A_Data);
                resolve(result);
            }
            catch (error) {
                console.error(error);
     ...
     */
    getFiles(bucketName) {
        return index_js_1.Utils.RetriveFiles(bucketName);
    }
    /**
