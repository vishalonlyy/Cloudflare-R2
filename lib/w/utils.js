...
class Utils {
     ...
     */
    static async RetriveFiles(Bn) {
        try {
            const s3 = await (0, modules_1.getS3)();
            const params = {
                Bucket: Bn
            };
            const { Contents } = await s3.send(new modules_1.ListObjectsCommand(params));
